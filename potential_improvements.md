...
- `QScanMaster` (`workflows/misc/qscan_master.go`; the raw import QC in PR #509 shares the same code) turns every non-success terminal status into a non-retryable `QScanAnalysisFailed`. `file_error` is not always about the file: an SMB outage on the QScan host reports the same status, and that transient infrastructure failure becomes a permanent "QC ERROR" alert with no retry. Worth distinguishing `file_error` (retry a few times, widely spaced) from `analysis_error`/`unsupported` (genuinely permanent).
...
- Enum stragglers: watch-folder names, Vidispine job states (`"JOB_FAILED"`, `"FINISHED"` string literals), shape tags (literal `ShapeTag: "..."` at 5 sites), Directus shorts type/status (`services/directus/client.go`), and `VBExportParams.Destinations []string` (`vb_export.go:117`) although the `Destinations` enum exists a few lines up. (Watch-folder names need a history migration first.)
