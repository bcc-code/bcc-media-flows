...
func PrepareFiles(ctx workflow.Context, params PrepareFilesParams) (*PrepareFilesResult, error) {
	...
	var videoTasks = map[string]workflow.Future{}
	{
		...
		for _, key := range keys {
			input := qualities[key]
			videoTasks[key] = wfutils.ExecuteWithQueue(ctx, utils.GetTranscodeQueue(), activities.TranscodeToVideoH264, input)
		}
	}
	...
	var audioTasks = map[string]workflow.Future{}
	{
		...
		for _, lang := range keys {
			...
			audioTasks[lang] = wfutils.ExecuteWithQueue(ctx, utils.GetAudioQueue(), activities.TranscodeToAudioAac, common.AudioInput{
				Path:            path,
				Bitrate:         "190k",
				DestinationPath: tempFolder,
			})
